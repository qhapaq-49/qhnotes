...
class PolicyValueNetwork(nn.Module):
    def __init__(self, blocks, channels, activation=nn.ReLU(), fcl=256, reduction=8):
        ...
        self.pos_emb_1 = Bias((FEATURES1_NUM, 9, 9))
        self.pos_emb_2 = Bias((FEATURES2_NUM, 9, 9))


    def forward(self, x1, x2):
        x1_in = self.pos_emb_1(x1)
        x2_in = self.pos_emb_2(x2)
        
        u1_1_1 = self.l1_1_1(x1_in)
        u1_1_2 = self.l1_1_2(x1_in)
        u1_2 = self.l1_2(x2_in)
        ...
        # policy scaling                                                                                                      
        h_max = torch.max(h_policy, dim=1).values.view(-1, 1)
        h_policy -= h_max
        h_policy = h_policy / (0.5 * sigmoid_scale)
        h_policy = 2.0 * sigmoid_scale * h_policy.sigmoid()

        # value scaling                                                                                                       
        h_value = h_value / (0.5 * sigmoid_scale)
        h_value = 2.0 * sigmoid_scale * h_value.sigmoid()
        h_value -= sigmoid_scale
